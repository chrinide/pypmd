...
c
      subroutine test (npoints) bind(c)
        use iso_c_binding, only: c_int
        implicit none
        integer(kind=c_int), value, intent(in) :: npoints
        write(*,*) npoints
      end subroutine test
...
c     Lebedev quadrature.
c
c-----Input:   
c     npaux = Number of (theta,phi) pairs we want.
c
c-----Ouput:   
c     npoints = Number of (theta,phi) actually used.
c     ct(i),st(i),cp(i),sp(i), i=1,npoints
c     w(i), i=1,npoints
c
c     (NPOINTS will be the closest integer from below to NPAUX 
c     of all the available Lebedev quadrature orders. See array index() 
c     in this routine)
c
c     ct(i),st(i),cp(i),sp(i) are the cos(theta), sin(theta), 
c     cos(phi), and sin(phi) values of the points of the quadrature.
c
c     w(i) are the weights of the quadrature.
c
      subroutine lebgrid (ct,st,cp,sp,w,npoints) bind(c)
c
c...............................................................
c
...
c
      ...
      real(kind=rp), parameter :: pi = 3.14159265358979323846264338328
c
      integer(kind=ip), value, intent(in) :: npoints
      real(kind=rp), intent(inout) :: ct(npoints)
      real(kind=rp), intent(inout) :: st(npoints)
      real(kind=rp), intent(inout) :: cp(npoints)
      real(kind=rp), intent(inout) :: sp(npoints)
      real(kind=rp), intent(inout) :: w(npoints)
c
      integer(kind=ip) :: n, i
      real(kind=rp) :: x(npoints)
      real(kind=rp) :: y(npoints)
      real(kind=rp) :: z(npoints)
      real(kind=rp) :: r, rxy
c
      n = 0_ip
...
c
      do i = 1,n
        ...
        if (rxy.lt.eps) then
          ...
          rxy = sqrt(rxy)
          ct(i) = z(i)/r
          st(i) = sqrt((1.0_rp-ct(i))*(1.0_rp+ct(i)))
          cp(i) = x(i)/rxy
          sp(i) = y(i)/rxy
        end if !
      end do
