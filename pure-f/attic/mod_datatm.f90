...
module mod_datatm
  ...
  use mod_param, only: bohrtoa
  implicit none
  public      

  integer(kind=ip), parameter :: maxzat = 118 ! maximum atomic number supported
  integer(kind=ip) :: i
  ...
  real(kind=rp), parameter :: wgatm(maxzat) = (/&
  ...
  real(kind=rp), parameter :: alpha_free(maxzat) = (/  0.6668D0,  0.2051D0, 24.3300D0,  5.6000D0,& ! 1-4
  ...
  real(kind=rp), parameter :: atmvdw(maxzat) = (/&
  ...
  ! Unit is Angstrom
  real(kind=rp) :: covr_suresh(maxzat) = (/&
  ...
  real(kind=rp) :: radii_hugo(maxzat) = (/&
