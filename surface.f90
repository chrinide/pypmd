subroutine surf_driver(nmo, nprims, icen, ityp, oexp, ngroup, nzexp, &
  ...
  use mod_surface, only: init_surf, rlimsurf_, nlimsurf_, minter, epsilon_, &
                      xnuc_, inuc_, xyzrho_, npang_, rotgrid_, rmaxsurf_, &
  ...
  real(kind=rp) :: rmaxs, rmins
  integer(kind=ip) :: i, lsu, ltxt, j, nchars
  ...
  !!$omp shared(npang,ct,st,cp,sp,epsilon_,rlimsurf,nlimsurf)
