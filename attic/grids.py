...
libcleb = misc.load_library('libcleb')
...
def gen_atomic_grids(self, atom_grid={}, radi_method=gauss_chebyshev, level=3, **kwargs):
    ...
    for ia in range(self.natm):
        ...
        if symb not in atom_grids_tab:
            ...
            angs = [n_ang] * n_rad
            logger.info(self,'atom %s rad-grids = %d, ang-grids = %s', symb, n_rad, angs)
            ...
            for n in sorted(set(angs)):
                ...
                libcleb.MakeAngularGrid(grid.ctypes.data_as(ctypes.c_void_p),
                                        ctypes.c_int(n))
